...
function buildPage() {
	...
	var note = E('div', { style: [
		...
		E('div', { style: 'color:var(--dy-muted);' }, [
			_('本节只配置 daed 的运行时参数。节点、订阅、路由规则、DNS 等代理配置请点「控制」页的「面板」按钮，在 daed Web UI 里配置（存储于 /etc/deer/daed.db，SQLite 状态库）。'),
		]),
	]);

	/* ── 表单 ── */
	m = new form.Map('deer', _('Deer (daed) 运行时设置'),
		_('修改后需重启 daed 才生效；点「保存并应用」会自动重启已运行的服务。'));
	...
	o = s.option(form.Value, 'listen', _('监听地址'),
		_('REST API 与 Web UI 的监听地址:端口。默认 0.0.0.0:2023（局域网可访问）；仅本机管理可改为 127.0.0.1:2023。'));
	...
	saveBtn.addEventListener('click', function () {
		...
		// m.save() 收集各控件值（含 depends 检查）→ ui.changes.apply() 提交到 UCI
		Promise.resolve(m.save())
	...
	return E('div', {}, [note, m.render(), actions]);
}

// ⚠️ 必须返回 LuCI Class（loader 会 `new _class()` 并检查 Class.isSubclass）；
// 返回普通对象字面量会报 `factory yields invalid constructor`。
return baseclass.extend({
	render: function () {
		var container = E('div', {}, [
			E('div', { style: 'font-size:12px;color:var(--dy-muted);padding:10px 0;' }, [_('加载中…')]),
		]);

		// form.Map 构造时只 load 了 'luci' 这个 config，deer 的 UCI 要自己加载，
		// 否则控件读不到当前值（显示为空）。
		L.resolveDefault(uci.load(NAME), null).then(function () {
			while (container.firstChild) container.removeChild(container.firstChild);
			try {
				container.appendChild(buildPage());
			}
			catch (err) {
				container.appendChild(E('div', { style: 'color:#e74c3c;font-size:12px;' },
					[_('表单渲染失败：%s').format(err)]));
			}
		});
